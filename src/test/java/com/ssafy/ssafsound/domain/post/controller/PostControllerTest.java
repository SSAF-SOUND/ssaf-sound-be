...
class PostControllerTest extends ControllerTest {
    ...
    @Test
    @DisplayName("전체 게시글 검색(Offset), 제목 + 내용을 기준으로 검색")
    void searchAllPostsByOffset() {
        doReturn(GET_POST_OFFSET_RES_DTO4)
                .when(postService)
                .searchAllPostsByOffset(any());

        restDocs.cookie(ACCESS_TOKEN)
                .when().get("/posts/all/search/offset?keyword={searchText}&page={page}&size={pageSize}", "싸피", 1, 10)
                .then().log().all()
                .statusCode(HttpStatus.OK.value())
                .apply(document("post/search-all-posts-by-offset",
                                requestCookieAccessTokenNeedless(),
                                requestParameters(
